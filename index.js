...
function customAssign(...objects) {
  ...
  let objNew = Object.assign(...objects);
  for (const key in objNew) {
    if (typeof objNew[key] === "number") objNew[key] = objNew[key].toString();
  }
...
function customObjectFromEntries(entries) {
  ...
  if (!Array.isArray(entries)) return "Вхідний аргумент має бути масивом.";

  let arr = entries.map(
    (k) =>
      typeof k[0] === "number" ? [(k[0] = k[0].toString()), (k[1] = k[0])] : k
    // або так
    // typeof k[0] === "number" ? [(k[0] = k[0].toString()), (k[1] = k[1].toString())] : k
  );
...
function filterObjectsByKey(arr, key) {
  ...
  arr = arr.filter((obj) => obj.hasOwnProperty(key));
  return arr;
}
...
function getObjectValuesSum(obj) {
  ...
  return arr.reduce((sum, elem) => (sum = sum + elem), 0);
}
...
function convertArrayToObj(arr) {
  ...
  obj = Object.fromEntries(arr); // зайвий рядок
  return obj;
}
